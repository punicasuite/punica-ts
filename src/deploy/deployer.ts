...
import { Address, deploy, initClient, isDeployed, reverseBuffer } from 'ontology-ts-test';
...
export class Deployer {
  async deploy(
    ...
    const sk = decodeAccount(account, password, wallet.scrypt);
    ...
    await deploy({
      client,
      account: {
        address: Address.fromBase58(account.address)
          .toArray()
          .toString('hex'),
        privateKey: sk
      },
